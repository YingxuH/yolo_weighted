...
def verify_image_label(args):
    ...
    im_file, lb_file, wt_file, prefix, keypoint = args
    ...
    try:
        ...
        # verify labels
        if os.path.isfile(lb_file) and os.path.isfile(wt_file):
            nf = 1  # label found
            ...
            with open(wt_file) as fw:
                weights = fw.readlines()
                weights = np.array([[float(w.strip())] for w in weights])
        ...
        lb = lb[:, :5]
        return im_file, lb, weights, shape, segments, keypoints, nm, nf, ne, nc, msg
