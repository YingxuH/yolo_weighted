...
# from src.weighted_base_dataset import BaseDataset
from src.utils import img2weight_paths
from src.yolo_utils import verify_image_label

from ultralytics.yolo.data.base import BaseDataset
from ultralytics.yolo.utils import NUM_THREADS, TQDM_BAR_FORMAT, is_dir_writeable
from ultralytics.yolo.data.augment import *
from ultralytics.yolo.data.utils import HELP_URL, LOCAL_RANK, get_hash, img2label_paths
...
class YOLODataset(BaseDataset):
    ...
    def cache_labels(self, path=Path("./labels.cache")):
        ...
        with ThreadPool(NUM_THREADS) as pool:
            results = pool.imap(func=verify_image_label,
                                iterable=zip(self.im_files, self.label_files, self.weight_files, repeat(self.prefix),
                                             repeat(self.use_keypoints)))
            pbar = tqdm(results, desc=desc, total=total, bar_format=TQDM_BAR_FORMAT)
            for im_file, lb, weights, shape, segments, keypoint, nm_f, nf_f, ne_f, nc_f, msg in pbar:
                nm += nm_f
                nf += nf_f
                ne += ne_f
                nc += nc_f
    ...
    def get_labels(self):
        ...
        self.weight_files = img2weight_paths(self.im_files)
    ...
    def collate_fn(batch):
        ...
        for i, k in enumerate(keys):
            ...
            if k in ["masks", "keypoints", "bboxes", "cls"]:
                ...
                if k == "cls":
                    if len(value) == 0:
                        new_batch["weights"] = torch.tensor([])
                    else:
                        new_batch["weights"] = value[:, 1]
                        value = value[:, 0]
            new_batch[k] = value
